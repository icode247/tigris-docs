...
Tigris applications follows a convention of having models and schemas in `models/tigris` directory. Within the directory we have todoStarterApp directory that will be our database name and the file todoItems.ts describes the schema for collection named todoItems.
