...
a [Collection](/overview/databases). Tigris applications follows a convention of having models and schemas in `models/tigris` directory. Within the directory you have `todoStarterApp` directory that will be our database name and the `todoItems.ts` file which describes the schema for collection named `todoItems`.
