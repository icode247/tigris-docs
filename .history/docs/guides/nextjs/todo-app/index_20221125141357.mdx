...
To update the records in the collection, create a add a `pages/api/item/[id].ts` to add `PUT` method which is routed to `api/item/[id] endpoint, where `id` is the unique
