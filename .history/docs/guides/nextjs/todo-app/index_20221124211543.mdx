...
a [Collection](/overview/databases). Tigris follows the convention of defining models and schemas in `models/tigris` directory. Within that directory, you'll need to another folder that matches the name of your database name. Then create your schema file.
Your models structure should look like `models/tigris/[DATABASE_NAME]/[SCHEMA_NAME].tsx`

follows a convention of having models and schemas in `models/tigris` directory. Within the directory you have `todoStarterApp` directory that will be our database name and the `todoItems.ts` file which describes the schema for collection named `todoItems`.
