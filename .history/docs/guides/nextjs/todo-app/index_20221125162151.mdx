...
To update the records in the collection, create a add a `pages/api/item/[id].ts` file to add `PUT` method which is routed to `api/item/[id] endpoint, where `id` is the unique
...
item in entirety with the new one. You can as well use the [`updateOne()`](/typescript/documents/update)
...
Now you can make a requet to the API to fetch and manipulate the from your Tigris database in your App components.
