...
// highlight-end
```
You've imported the Tigris `TigrisCollectionType` type, which is extended to define a model for your collections, the `TigrisDataTypes` type that gives you access to the Tigris data types, and the `TigrisSchema` which is usde to define database schema for y.
