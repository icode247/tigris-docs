...
Once the script runs, a database named blogApp (derived from directory name) and a collection named blogItems (derived from filename) will be created conforming to TodoItemSchema. If the collection already exists, it will apply schema changes, if any, to that collection.
