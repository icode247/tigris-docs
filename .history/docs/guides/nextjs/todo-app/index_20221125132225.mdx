...
Now automate your schema management to configure how the Tigris instance loader will be executed at runtime and register your schema. Create a `sripts/setup.ts` file in the root directory of your project and add the code:
...
main();
```
Here you used the `registerSchemas()` method which accepts the path to the databases and collections that needs to be created. Then we all the models in the `models/tigris` directory with
Tigris client that includes a `blogApp` directory and `blogItems.ts` file with the `TodoItemSchema` you
defined.

Once the script runs, A database named todoStarterApp (derived from directory name) and a collection named todoItems (derived from filename) will be created conforming to TodoItemSchema. If the collection already exists, it will apply schema changes, if any, to that collection.
