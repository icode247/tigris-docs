...
Replace `<YOUR_PROJECT_NAME>` and `<YOUR_PROJECT_DESCRIPTION>` arguments with your project name and description, respectively. This will create the credentials and print them on the terminal screen:
...
// highlight-end
```
You have imported the Tigris `TigrisCollectionType` type, which is extended to define a model for your collections, the `TigrisDataTypes` type that gives you access to the Tigris data types, and the `TigrisSchema` that gives you access to the Tigris data types.
Once these scripts run, a database named todoStarterApp (derived from the directory name) and a collection called `todoItems` (derived from the filename) will be created, conforming to TodoItemSchema. If the collection already exists, it will apply schema changes, if any, to that collection.
...
Once the script runs, a database named `blogApp` (derived from the directory name) and a collection called `blogItems` (derived from the filename) will be created, conforming to BlogItemSchema. If the collection already exists, it will apply schema changes, if any, to that collection.
...
#### ⌲ POST `/api/items` to add a new item to the collection.
...
To update the records in the collection, create an add a `pages/api/item/[id].ts` file to add `PUT` method, which is routed to `api/item/[id] endpoint, where `id` is the unique
the identifier for the item that is being updated:
...
Similar to PUT method above, we will now add a `DELETE` method to the `pages/api/item/[id].ts` file to
...
Now you can request the API to fetch and manipulate the data in your Tigris database in your App components.
