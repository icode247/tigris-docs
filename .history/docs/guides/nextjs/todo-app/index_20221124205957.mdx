...
This guide will teach you how to add Tigris to your existing Nextjs project.
This guide discusses the concepts behind using Tigris in your existing Nextjs project, explains the commonalities and differences between MongoDB and other database providers, and leads you through the process for configuring your application to integrate with MongoDB using Prisma.
...
a [Collection](/overview/databases). The generated starter app has a single collection `todoItems` that
stores to-do items as documents.

Tigris applications follows a convention of having models and schemas in `models/tigris` directory. Within the directory you have `todoStarterApp` directory that will be our database name and the `todoItems.ts` file which describes the schema for collection named `todoItems`.

Add the highlighted code to `models/tigris/todoStarterApp/todoItems.ts` file:
